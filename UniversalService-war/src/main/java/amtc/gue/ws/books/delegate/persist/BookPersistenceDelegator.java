	...
	public IDelegatorOutput delegate() {
		...
		if (persistenceInput.getType().equals(PersistenceTypeEnum.ADD)) {
			...
			if (persistenceInput.getInputObject() instanceof Tags) {
				...
				Tags tags = (Tags) persistenceInput.getInputObject();
				...
				try {
					for (String tagName : tags.getTags()) {
						books.addAll(bookEntityDAO.getBookEntityByTag(tagName));
					}

					log.info(
							"Books retrieved for tag '" + persistenceInput.getInputObject() + "': " + books.toString());
